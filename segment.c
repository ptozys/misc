...
void f2fs_steal_inmem_pages_all(struct f2fs_sb_info *sbi, struct inode *inode)
{
	...
	list_for_each_entry_safe(page_cur, page_tmp, &afs->inmem_pages_list, list) {
		...
		if (!page_cur)
			printk("[JATA DBG] (%s) page_cur is NULL\n", __func__);
	...
	if (unlikely(!writeback_in_progress(&sbi->sb->s_bdi->wb)))
		f2fs_wb_start_background_writeback(&sbi->sb->s_bdi->wb);
	return;
}
...
static int __f2fs_commit_inmem_pages_atomic_file_set(struct inode *inode,
                                                     struct list_head *revoke_list)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	struct f2fs_inode_info *fi = F2FS_I(inode);
...
int f2fs_commit_inmem_pages_atomic_file_set(struct inode *inode,
                                            struct list_head *revoke_list)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	struct f2fs_inode_info *fi = F2FS_I(inode);
